...
one generated sex-offender-registry adapter per covered country. Supplies the
domain bindings and Git-synchronized adapter output so fleet workers land
verified code before publishing durable coverage.
"""
...
LEDGER_REL = "docs/country_coverage.csv"
ADAPTERS_REL = "adapters_generated"
LEDGER_PATH = PROJECT_ROOT / LEDGER_REL
ADAPTERS_OUT = PROJECT_ROOT / ADAPTERS_REL
...
def build_spec() -> ExpandSpec:
    ...
    return ExpandSpec(
        ...
        scrubber_paths=[ADAPTERS_REL],
        ledger_rel="" if isinstance(ledger, PostgresCoverageLedger) else LEDGER_REL,
        env_prefix=ENV_PREFIX,
        system_prompt=_system_prompt(),
        git=GitSync(PROJECT_ROOT),
        on_discard=_discard,
